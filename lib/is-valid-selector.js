 ...
 * and tested SelectorPatternObjects that the user can reference by name.
...
function isValidSelector(selector, componentName, isStrictMode, patternDef) {
  ...
  var pattern = (typeof patternDef === 'string')
    ? selectorPatterns[patternDef]
    : patternDef;
  var componentPattern = (pattern.component) ? pattern.component(componentName) : pattern(componentName);
  var chainablePattern = (pattern.chainable) ? pattern.chainable(componentName) : pattern(componentName);
